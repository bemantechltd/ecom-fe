<template>
    <div class="product_info_by_company">
        <label>
            <span>Company wise products</span>
        ...
        <div v-else-if="contents.length>0">
            <!-- <GChart
                ...
                /> -->
            <PieChart :chartOptions="canvasChartOptions" :content_type="'ProductCompany'" ref="pie_chart" />
        </div>
...
export default {
    name: 'ProductInfoByCompanyBlock',
    components: {
        PieChart
    },
    data(){
        return {
            ...
            chartOptions: {
                chart: {
                    title: 'Total products by company',
                    subtitle: 'Company and total post'                    
                },
                // is3D: true,
                // pieHole: 0.4,
                slices: {},
    ...
    methods: {
        async load_data(){
            let url = '/api/report-data/products-by-company';
            ...
            this.$axios.get(url, this.$parent.header_config).then( (response) => {
                ...
                if(response.data.data.length>0){
                    ...
                    this.chartData[i++] = ['Company','Total Post'];
                    ...
                    this.contents.forEach(v => {                        
                        ...
                        if(v.company) this.chartData[i++] = [v.company.company_name,get_total_post];
...
<style scoped>
    ...
    .product_info_by_company{
        display: block;
    }
    .product_info_by_company > div{
        display: block;
        min-height: 200px;
    ...
    .product_info_by_company > div.loader_block,
    .product_info_by_company > div.empty_block{
        text-align: center; padding: 35px 0; color: #999;
    }
    .product_info_by_company > div.loader_block > i,
    .product_info_by_company > div.empty_block > i{
