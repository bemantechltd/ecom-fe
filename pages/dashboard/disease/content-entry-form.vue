            ...
            <div class="col-md-8">
                ...
                <div v-else class="form_block">
                    <div class="mb-4">
                        ...
                        <div><input type="text" placeholder="Enter disease title" class="form-control" v-model="formData.disease_title" @keyup="slug_config" ref="disease_title" /></div>
                    </div>
                    <div class="mb-4">
                        <label>Slug</label>
...
export default {
    ...
    methods: {
        ...
        slug_config(){            
            this.formData.slug = this.$strSlug(this.formData.disease_title)
        },
        async load_req_data(id){
            this.form_loader = true;
            
            this.$axios.get('/api/disease/edit/' + id, this.$parent.header_config).then( (response) => {
                console.log('Get Data', response.data)
                let getData = response.data;

                this.formData = {
                    disease_title: getData.disease_title,                
                    slug: getData.slug,                    
        ...
        async formSubmit(){
            ...
            if(this.formData.disease_title.trim()==''){
                this.$toast.error('Please enter disease name', {icon: "Warning"});
                this.$refs.disease_title.focus();
                return false;
            ...
            if(confirm('Are you sure to submit it?')){
                ...
                await this.$parent.diseaseSubmit(submit_data);
