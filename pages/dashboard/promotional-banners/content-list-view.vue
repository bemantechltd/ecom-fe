            ...
            <tbody>
                <template v-if="data.length>0">
                    <tr v-for="(item,index) in data" :key="index">
                        ...
                        <td>
                            <span v-if="item.desktop_banner_image" class="desktop_banner">
                                <img :src="item.desktop_banner_image" />                
                            </span>
                        </td>
                        <td>
                            <span v-if="item.mobile_banner_image" class="mobile_banner">
                                <img :src="item.mobile_banner_image" />                
                            </span>
                        </td>
                        <td>
                            <div>Display Type: {{ $store.state.banner_display_types[item.display_type] }}</div>
                            <div>Schedule Type: {{ $store.state.banner_schedule_types[item.schedule_type] }}</div>
                        </td>
                        <td align="center">
                            <span class="status">
    ...
    methods: {
        ...
        del_item: function(id){
            if(confirm('Are you sure to delete it?')){
                this.$axios.get('/api/promotional-banners/delete/' + id, this.$parent.header_config).then( (response) => {
...
<style scoped>
    ...
    .mobile_banner{
        ...
        width: 150px; height: 67px; padding: 5px;
