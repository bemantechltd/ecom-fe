...
export default {
    ...
    methods: {
        ...
        async load_data(pg=this.cur_page){
            let url = '/api/customer-infos'+ (this.limit>0?'?limit=' + this.limit:'') + (pg>1?'&page=' + pg:'') + (this.$parent.$parent.role_access.view_others=='0'?'&own_result=true':'') + (this.$store.state.search_keyword?'&keyword=' + this.$store.state.search_keyword:'') + (this.filter.status?'&status=' + this.filter.status:'') + (this.filter.verified?'&verified=' + this.filter.verified:'')  + (this.filter.date_range?'&date_range=' + this.filter.date_range:'');
