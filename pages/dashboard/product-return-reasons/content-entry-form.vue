            ...
            <div class="col-md-8">
                ...
                <div v-else class="form_block">
                    <div class="mb-4">
                        ...
                        <div><input type="text" placeholder="Enter reason title" class="form-control" v-model="formData.reason_title" ref="reason_title" /></div>
                    </div>
                    <div class="mb-4">
                        <label>Need Details</label>
                        <SwithcBtn :status="formData.dtl_req" :index="'dtl_req'" />
                        <input type="hidden" v-model="formData.dtl_req" />
                    </div>
                </div>
            </div>
            <div class="col-md-4">
...
export default {
    ...
    methods: {
        ...
        async load_req_data(id){
            ...
            this.$axios.get('/api/product-return-reason/edit/' + id, this.$parent.header_config).then( (response) => {
                console.log('Get Data', response.data)
                let getData = response.data;

                this.formData = {
                    reason_title: getData.reason_title,
                    dtl_req: getData.dtl_req,
        ...
        async formSubmit(){
            ...
            if(this.formData.reason_title.trim()==''){
                this.$toast.error('Please enter reason title', {icon: "Warning"});
                this.$refs.reason_title.focus();
                return false;
            }
            ...
            if(confirm('Are you sure to submit it?')){
                ...
                await this.$parent.productReturnReasonSubmit(submit_data);
