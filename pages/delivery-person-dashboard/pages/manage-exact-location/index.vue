    ...
    <div :class="['manage_shipping_address_block',{mobile_device:$device.isMobile}]">
        ...
        <div align="right" class="pr-3">            
            <template v-if="entry_form_open">
                <span class="action_btn cancel mr-2" @click="entry_form_open=false"><i class="fa fa-times mr-1"></i> Cancel</span>
                    ...
                    <div class="col-md-7 mb-3" align="left">
                        <template v-if="contents.length>0">
                            <div class="default_setup_section">
                                <span :class="[{active:sel_default_setup_index==1},{inactive:sel_default_setup_index==2}]" @click="sel_default_setup(1)"><i class="fa fa-check-circle"></i> Setup default address</span>
                            </div>
                        </template>
                    </div>
                    <div class="col-md-5 mb-3">
                        <template v-if="sel_default_setup_index">
                            <span class="action_btn cancel mr-2" @click="sel_default_setup_index=''"><i class="fa fa-times mr-1"></i> Cancel</span>
                            <span class="action_btn save" @click="$refs.list_block.submit_default_setup"><i class="fa fa-save mr-1"></i> Save</span>
                            <span v-if="req_default_setup_index && $refs.list_block.sel_address_data" class="action_btn select ml-2" @click="$refs.list_block.select_address"><i class="fa fa-check mr-1"></i> Select</span>
                        </template>
                        <span v-else class="action_btn new" @click="form_open"><i class="fa fa-plus-circle mr-1"></i> Add new exact location</span>
                    </div>
                </div>
            </template>
        </div>
        ...
        sel_default_setup: function(val){
            ...
            if(this.contents.length>0){
                this.contents.forEach(v => {
                    if(this.sel_default_setup_index==1 && v.default_shipping_address){
