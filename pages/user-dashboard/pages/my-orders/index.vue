<template>
    <div class="manage_orders_block">
        ...
        <RefundModalView v-if="open_refund_modal" :data="refund_product_item" :order_id="sel_order_id" />
    </div>
</template>
<script>
...
import RefundModalView from '@/components/content_display/RefundModal';
...
export default {
    ...
    methods: {
        ...
        view_refund_modal: function(data,order_id){
            this.open_refund_modal = true
            this.refund_product_item = data
            this.sel_order_id = order_id
        },
        async load_return_reasons(){
            let url = '/api/product-return-reason';

            this.return_reason_types = [];
            this.$axios.get(url, this.header_config).then( (response) => {
                console.log(response)
                this.return_reason_types = response.data.data
            }).catch(e => {
        ...
        close_refund_modal: function(){
            this.open_refund_modal = false
            document.body.classList.remove('popup_open')
        }
    }
}
