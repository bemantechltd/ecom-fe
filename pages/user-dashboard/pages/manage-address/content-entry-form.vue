<template>
    <div class="content_entry_form_block">
      <!-- <pre>{{ $store.state.auth_info.user_data }}</pre> -->
                ...
                <div class="col-md-12 mb-3">
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            <label>Region</label>
                            <DataInfoForm :column="'region_id'" :type="'dropdown'" :data="regions" :vf="'id'" :tf="'title'" :isize="'full'" :onchange="'load_cities'"  />
                        </div>
                        <div class="col-md-4 mb-3">
                            <label>City</label>
                            <DataInfoForm :column="'city_id'" :type="'dropdown'" :data="cities" :vf="'id'" :tf="'title'" :isize="'full'" :onchange="'load_areas'" />
                        </div>
                        <div class="col-md-5 mb-3">
                            <label>Area</label>
                            <DataInfoForm :column="'area_id'" :type="'dropdown'" :data="areas" :vf="'id'" :tf="'title'" :isize="'full'" />
                        </div>
                        <div class="col-md-5 mb-3">
                            <label>Address</label>
...
export default {
    ...
    async mounted(){
        ...
        if(this.edit_content_id) this.load_req_data(this.edit_content_id);
          ...
          this.formData.full_name = this.$store.state.auth_info.user_data.user_info.full_name?this.$store.state.auth_info.user_data.user_info.full_name:''
          this.formData.contact_no = this.$store.state.auth_info.user_data.mobile?this.$store.state.auth_info.user_data.mobile:''
        }
    ...
    methods: {
        ...
        async submit_form(){
            ...
            if(getFullName.trim()==''){
                ...
                this.$toast.error('Please choose destination', {icon: "Warning"});
                return false;
            }
            ...
            if(confirm('Are you sure to submit it?')){
                ...
                this.$axios.post('/api/manage-address/' + (this.edit_content_id?'update/' + this.edit_content_id:'store'), this.formData, this.$parent.header_config).then( async (response) => {
                    ...
                    await this.$swal("Good job!", "Address has been added successfully.", "success")
